...
function getRangeModifier(srcNode, rRoll, nRange)
	...
	if ActorManager.isPC(srcNode) then
		...
		local tWeaponRangeData = DataCommonCombatModifiers.weaponranges[sWeaponUsed];
		if tWeaponRangeData == nil then
			...
			for sWeaponName, tWeaponRangeData in pairs(DataCommonCombatModifiers.weaponranges) do
